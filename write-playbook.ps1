...
$target = "pyslick_package\pyslick_pkg\pyslick\playbook.py"
...
# ── Backup ───────────────────────────────────────────────────────────────
$backupDir = Join-Path $here ".pyslick\backups"
if (-not (Test-Path -LiteralPath $backupDir)) {
    New-Item -ItemType Directory -Path $backupDir -Force | Out-Null
}
$ts = Get-Date -Format 'yyyyMMdd-HHmmss-fff'
$backup = Join-Path $backupDir "playbook.py.$ts.bak"
Copy-Item -LiteralPath $target -Destination $backup -Force
Write-Host "Backup: $backup" -ForegroundColor Cyan

# ── Playbook body (raw strings so backslashes survive) ───────────────────
$playbook = @'
"""
...
[System.IO.File]::WriteAllText((Resolve-Path -LiteralPath $target).Path, $playbook, $enc)
...
if ($bytes.Length -ge 3 -and $bytes[0] -eq 0xEF -and $bytes[1] -eq 0xBB -and $bytes[2] -eq 0xBF) {
    ...
    Write-Host "FAIL: BOM written. Rolled back." -ForegroundColor Red
    exit 1
}

$verify = [System.IO.File]::ReadAllText((Resolve-Path -LiteralPath $target).Path, (New-Object System.Text.UTF8Encoding($false, $true)))

if (-not $verify.Contains("PLAYBOOK_TEXT = r")) {
    Copy-Item -LiteralPath $backup -Destination $target -Force
    Write-Host "FAIL: PLAYBOOK_TEXT missing. Rolled back." -ForegroundColor Red
    exit 1
}
if (-not $verify.Contains("HANDOFF_TEXT = r")) {
    Copy-Item -LiteralPath $backup -Destination $target -Force
    Write-Host "FAIL: HANDOFF_TEXT missing. Rolled back." -ForegroundColor Red
    exit 1
}
if (-not $verify.Contains("The only rule that never changed")) {
    Copy-Item -LiteralPath $backup -Destination $target -Force
    Write-Host "FAIL: playbook body missing. Rolled back." -ForegroundColor Red
    exit 1
}
if ($verify.Contains([char]0xFFFD)) {
    Write-Host "WARN: replacement char (U+FFFD) in file." -ForegroundColor Yellow
}

Write-Host "Verified: PLAYBOOK_TEXT + HANDOFF_TEXT landed, no BOM, no mojibake." -ForegroundColor Green

# ── Python syntax check ──────────────────────────────────────────────────
Push-Location (Split-Path -Parent $target)
try {
    $pyOut = & python -c "import sys; sys.path.insert(0, '.'); import playbook; print('import OK, playbook len:', len(playbook.PLAYBOOK_TEXT), 'handoff len:', len(playbook.HANDOFF_TEXT))" 2>&1
...
Write-Host ""
Write-Host "Done. Undo: Copy-Item '$backup' '$target' -Force" -ForegroundColor Cyan
exit 0