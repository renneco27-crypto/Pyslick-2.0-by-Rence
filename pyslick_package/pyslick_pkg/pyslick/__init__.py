...
⚠ CRITICAL RULE FOR AI AGENTS & DEVELOPERS:
  NEVER EVER scan the entire codebase line-by-line!
  Use `pyslick graphify` to instantly know how data flows and which function
...
━━  AST Call-Graph & Data-Flow Engine (Zero Codebase Line Scanning)  ━━━━━━━━━━

  graphify [extract] [path]
      Index or refresh the full AST call-graph across all source files
      (.py, .ts, .tsx, .js, .go, .rs, .java) into graphify-out/graph.json.
      Example:  pyslick graphify extract .
      
  graphify query "<symbol_or_question>"   (or: pyslick graphify "<query>")
      Search codebase functions/classes. Displays signature, line range, source
      code, callers (who calls it), and callees (what it calls).
      Example:  pyslick graphify "createBrowserSupabase"
      Example:  pyslick graphify query "handleAuthCallback"

  graphify <file> [query]
      Inspect functions and caller/callee connections inside a specific file.
      Example:  pyslick graphify src/sw.ts
      Example:  pyslick graphify graphify.py "extract symbols"

  graphify flow <from_func> <to_func>
      Trace the exact step-by-step data flow & BFS call chain between functions.
      Example:  pyslick graphify flow repomap relations

  graphify callers <func_name> / callees <func_name>
      List all callers (who calls it) or callees (what it calls) for a function.
      Example:  pyslick graphify callers scan_imports_and_dependencies
      Example:  pyslick graphify callees build_repo_map

  graphify stats
      Display total mapped functions, call connections, and top God-Node hubs.
      Example:  pyslick graphify stats
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Quick Start:
    pyslick graphify "what does this codebase do"
    pyslick graphify flow repomap relations
            ...
            if command in ("--help", "-h"):
                ...
                try:
                    ...
                    if len(cleaned_args) >= 2 and os.path.isfile(cleaned_args[0]):
                        ...
                        mode_semantic_grep(query_str, root=target_dir, context=context, exact=exact, stream=stream)
                except Exception as e:
                    print(f"Error: {e}")
                    sys.exit(1)
                ...
                try:
                    from graphify import cli_main as graphify_cli_main
                    graphify_cli_main(args)
                except Exception as e:
                    print(f"Error: {e}")
                    import traceback
                    traceback.print_exc()
